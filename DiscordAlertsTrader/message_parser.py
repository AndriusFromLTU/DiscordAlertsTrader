...
def parse_trade_alert(msg, asset=None):
    ...
    # Detect market shorthand '@MKT' or '@MKT add' etc.; we will force price=None
    market_flag = False
    ...
    if match:
        ...
        if match:
            ...
            order = {"action": "ExitUpdate", "Symbol": symbol, "asset": asset_type}
...
def parse_exits_vals(msg, expr):
    re_comp = re.compile(
        "\s" + expr + "[:]?[ ]*[$]*(\d*[\.]*[\d]*[%]?)(TS[\d+\.]*[%]?)?", re.IGNORECASE
    )
    exit_inf = re_comp.search(msg)

    if exit_inf is None:
        re_comp = re.compile(
            "(\s" + expr.lower() + "[:]?[ ]*[$]*(\d*[\.]*[\d]*[%]?))", re.IGNORECASE
        )
        exit_inf = re_comp.search(msg)
...
def parse_sell_ratio_amount(msg, asset):
    ...
    if any(
        subs in msg.lower()
        for subs in ["half off my remaining position", "selling half off"]
    ):
        return 0.25
...
def parse_symbol(symbol: str):
    ...
    match = re.match(
        r"^([A-Z]+)_(\d{2})(\d{2})(\d{2})([CP])((?:\d+)(?:\.\d+)?)", symbol
    )
