    ...
    def price_now(self, symbol, price_type="BTO", pflag=0):
        ...
        try:
            ...
            if (
                ...
                or (
                    resp[symbol].get("bidPrice") == 0
                    and resp[symbol].get("askPrice") == 0
                )  # Zero prices indicate invalid/expired
            ):
                ...
                # Double-check the specific price type isn't zero
                if quote == 0:
                    str_msg = f"{symbol} has zero {ptype}"
                    print(Back.RED + str_msg)
                    self.queue_prints.put([str_msg, "", "red"])
                    quote = -1
    ...
    def new_trade_alert(self, order: dict, pars: str, msg):
        ...
        if order["action"] == "ExitUpdate" and isOpen:
            ...
            if pd.isnull(self.portfolio.loc[open_trade, "Avged"]):
                ...
                self.portfolio.loc[open_trade, "Avged-prices-alert"] = (
                    f"{al_pr},{alert_price}"
                )
                self.portfolio.loc[open_trade, "Avged-prices"] = (
                    f"{av_pr},{order_info['price']}"
                )
                self.portfolio.loc[open_trade, "Avged-Qty"] = (
            ...
            if trade["BTO-Status"] == "invTSbuy":
                ...
                if quote_opt <= stp_price * 1.01:  # 1%
                    ...
                    if quote_opt > max_price:
                        max_price = quote_opt
                        self.portfolio.loc[i, "open_trailingstop"] = (
                ...
                for avg in avg_down_list:
                    ...
                    if (
                        ...
                        if pd.isnull(self.portfolio.loc[i, "Avged"]):
                            ...
                            self.portfolio.loc[i, "Avged-prices-alert"] = (
                                f"{al_pr},{order['price']}"
                            )
                            self.portfolio.loc[i, "Avged-prices"] = (
                                f"{av_pr},{order_info['price']}"
                            )
                            self.portfolio.loc[i, "Avged-Qty"] = (
                                f"{av_qt},{order_info['quantity']}"
                            )
