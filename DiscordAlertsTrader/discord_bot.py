...
class DiscordBot(discord.Client):
    def __init__(
        ...
        if brokerage is not None:
            self.trader = AlertsTrader(
                queue_prints=self.queue_prints, brokerage=brokerage, cfg=self.cfg
            )
        self.tracker = AlertsTracker(
            brokerage=brokerage, portfolio_fname=tracker_portfolio_fname, cfg=self.cfg
        )
        self.load_data()

        if (live_quotes and brokerage is not None and brokerage.name != "ibkr") or (
            brokerage is not None
            and brokerage.name == "ibkr"
            and cfg["general"].getboolean("ibkr_live_quotes")
        ):
    ...
    def track_live_quotes(self):
        ...
        while self.live_quotes:
            ...
            tk_day = (
                pd.to_datetime(self.tracker.portfolio["Date"]).dt.date == date.today()
            )
            td_day = (
                pd.to_datetime(self.trader.portfolio["Date"]).dt.date == date.today()
            )
            ...
            track_symb = set(
                self.tracker.portfolio.loc[msk_tk, "Symbol"].to_list()
            ...
            for q in quote:
                if (
                    quote[q].get("description") == "Symbol not found"
                    or q == ""
                    or quote[q]["bidPrice"] == 0
                ):
                    continue
                ...
                if (
                    len(last_line)
                    and float(last_line.split(",")[1]) == quote[q]["bidPrice"]
                    and float(last_line.split(",")[2]) == quote[q]["askPrice"]
                ):
                    continue
                ...
                with open(file_path, "a+") as f:
                    ...
                    f.write(
                        f"{timestamp}, {quote[q]['bidPrice']}, {quote[q]['askPrice']}\n"
                    )
    ...
    def load_data(self):
        ...
        for ch in self.channel_IDS.keys():
            ...
            if not os.path.exists(dt_fname):
                ch_dt = pd.DataFrame(
                    columns=self.cfg["col_names"]["chan_hist"].split(",")
                )
                ch_dt.to_csv(dt_fname, index=False)
                ch_dt.to_csv(
                    f"{self.cfg['general']['data_dir']}/{ch}_message_history_temp.csv",
                    index=False,
                )
            else:
    ...
    async def on_message(self, message):
        ...
        author = f"{message.author.name}#{message.author.discriminator}".replace(
            "#0", ""
        )
        ...
        if message.channel.id == int(cfg["discord"]["commands_channel"]):
            ...
            message.channel.id not in self.channel_IDS.values()
            and author.lower()
            not in split_strip(self.cfg["discord"]["authorwise_subscription"])
        ):
            return
    ...
    def new_msg_acts(self, message, from_disc=True):
        if from_disc:
            msg_date = message.created_at.replace(tzinfo=timezone.utc).astimezone(
                tz=None
            )
            ...
            msg = message
        chn = msg["Channel"]
        shrt_date = datetime.strptime(msg["Date"], self.time_strf).strftime(
            "%Y-%m-%d %H:%M:%S"
        )
        ...
        if pars is None:
            if self.chn_hist.get(chn) is not None:
                ...
                self.chn_hist[chn] = pd.concat(
                    [self.chn_hist[chn], msg.to_frame().transpose()],
                    axis=0,
                    ignore_index=True,
                )
            ...
            if (
                live_alert
                and self.bksession is not None
                and (order.get("price") is not None)
            ):
                quote = self.trader.price_now(order["Symbol"], order["action"], pflag=1)
                act_diff = -1

                # Check for failed quote lookup (returns -1)
                if quote == -1:
                    str_msg += " (quote unavailable)"
                    self.queue_prints.put([f"\t {str_msg}", "yellow"])
                    print(Fore.YELLOW + f"\t {str_msg}")
                    # Skip BTO/STO if quote unavailable (safety check)
                    if order.get("action") in ["BTO", "STO"]:
                        return
                    # Allow exits even without quotes (positions may need closing)
                    ...
                    if abs(act_diff) > 1 and order.get("action") == "BTO":
                        ...
                        if self.chn_hist.get(chn) is not None:
                            ...
                            self.chn_hist[chn].to_csv(
                                self.chn_hist_fname[chn], index=False
                            )
                        return
    ...
    def do_trade_alert(self, author, channel, order):
        ...
        if author.lower() in split_strip(
            self.cfg["discord"]["authors_subscribed"]
        ) or channel.lower() in split_strip(
            self.cfg["discord"]["channelwise_subscription"]
        ):
            # ignore if no STC
            if (
                not self.cfg["general"].getboolean("DO_STC_TRADES")
                ...
                if order["action"] == "BTO" and order["asset"] == "option":
                    ...
                    if len(min_price) and order["price"] * 100 < float(
                        cfg["order_configs"]["min_opt_price"]
                    ):
                        str_msg = (
                            f"Option price is too small as per config: {order['price']}"
                        )
                        print(Fore.GREEN + str_msg)
                        self.queue_prints.put([str_msg, "", "green"])
                        return False, order
        ...
        # in authors shorting list
            ...
            if order["action"] in ["BTC", "STO"] and channel in [
                "GUI_user",
                "GUI_both",
            ]:
                return True, order
            ...
            if (
                order["action"] == "BTC"
                and not self.cfg["shorting"].getboolean("DO_BTC_TRADES")
            ) or (
                order["action"] == "STO"
                and not self.cfg["shorting"].getboolean("DO_STO_TRADES")
            ):
                return False, order
