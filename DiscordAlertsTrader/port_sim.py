...
def get_hist_quotes(
    symbol: str, date_range: List[date], client, interval_size: int = 1000
):
...
def port_cap_trades(
    ...
    if max_dte is not None or min_dte is not None:
        de = (
            data.loc[data["Asset"] == "option", "Symbol"]
            .str.extract(r"_(\d{6})")
            .iloc[:, 0]
        )
        de = pd.to_datetime(de, format="%m%d%y").dt.date
        dte = pd.to_timedelta(
            de - pd.to_datetime(data["Date"], format="mixed").dt.date
        ).dt.days
        # msk out, neg so NaNs are not removed
    ...
    if max_trade_val is not None:
        ...
        data.loc[exceeds_cap, "Qty"] = np.floor(
            max_trade_val / (data["Price"] * option_mult)
        )
    ...
    if any([max_u_qty, max_trade_val]):
        ...
        data.loc[:, "PnL$-actual"] = (
            data["Qty"] * data["PnL-actual"] * data["Price-actual"] * mult
        )
...
def filter_data(
    ...
    if filt_date_frm:
        ...
        msk = (
            pd.to_datetime(data["Date"]).dt.date >= pd.to_datetime(filt_date_frm).date()
        )
        data = data[msk]
    if filt_date_to:
        ...
        msk = (
            pd.to_datetime(data["Date"]).dt.date <= pd.to_datetime(filt_date_to).date()
        )
        data = data[msk]
    if filt_hour_frm:
...
def calc_roi(
    ...
    if avgdown is not None:
        ...
        if len(ds_inf):
            ...
            initial_price = (
                sum([initial_price] + [i[1] * i[2] for i in ds_inf]) / tot_qty_ratio
            )
